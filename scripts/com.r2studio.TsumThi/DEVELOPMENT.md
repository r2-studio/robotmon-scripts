...
controller. Separately, `gPause` (in `state.ts`) offers a *mid-task* pause driven
by a file on disk, checked at safe points inside long loops.
...
| `taskController.ts` | `TsumTaskController` — the cooperative scheduler. `newTask(name, fn, interval, runTimes)` registers work; `loop()` repeatedly runs the highest-priority *due* task. Ties break toward the longer interval, then the least-recently-run. Also holds the `errorCount` guard and the between-task `gPause.gate()` call. |
| `state.ts` | The two script-wide globals `ts` (the live `Tsum`) and `gTaskController`, plus `gPause`: a file-backed pause flag (`<storage>/tsum_pause`) a PC hotkey can write over adb, letting you pause *mid-game* rather than only between tasks. |
...
- `tools/pause-hotkey.ahk` is the PC side of `gPause`: it writes the
  `tsum_pause` flag over adb so you can freeze the script mid-game from the
  keyboard and inspect the screen.
