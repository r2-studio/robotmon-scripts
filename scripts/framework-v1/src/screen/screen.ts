...
export class Screen {
  ...
  public constructor(config: ScreenConfig) {
    ...
    this.config.deviceHeight = deviceWH.height;
    this.config.deviceWidth = deviceWH.width;
    // default, can change by calculateDeviceOffset
    this.config.screenWidth = deviceWH.width;
    this.config.screenHeight = deviceWH.height;
    this.config.screenOffsetX = 0;
    this.config.screenOffsetY = 0;
  }
  ...
  public isSameColor(devColorPoint: IXYRGB, thres: number = 0.9): boolean {
    const rgb = this.getScreenColor(devColorPoint);
    const score = Colors.identityColor(rgb, devColorPoint);
    if (score > thres) {
      return true;
    }
    return false;
