...
// @brief find page, return page's name
// @param times keep on same page for 0.1*times seconds
// @param timeout maximum detect time in ms
...
// @brief sleep till (prevMS + ms)
// @return current time in ms.
Tsum.prototype.sleep = function(t, prevMS) {
  if (t == undefined) {
    t = 1000;
  }
  if (prevMS !== undefined) {
    diff = t - (Date.now() - prevMS);
    if (diff <= 0) {
      return Date.now();
    }
    t = diff;
...
function start(isJP, detect, autoLaunch, detectAppOnPeriod, autoPlay,
    ...
    sendHeartMaxDuring, sendHeartsInterval, isLocaleTW) {
