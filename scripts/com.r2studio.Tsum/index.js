...
function loadTsumRotationImages(tsumMaxScores, debug) {
  var tsumPath = getStoragePath() + '/' + Config.tsumDir;
...
Tsum.prototype.init = function() {
  ...
  this.playOffsetY = (this.gameWidth - this.gameWidth) * 0.6;
...
// ============== manage tsum images ===============
Tsum.prototype.prepareTsum = function(boardImg) {
  this.releaseRotationTsum();

  if (this.debug) {
    saveImage(boardImg, getStoragePath() + "/tmp/boardImg.jpg");
  }
  this.gameTsums = findAllTsumMatchScore(this.allTsumImages, boardImg, this.myTsum);
  this.gameTsums = this.gameTsums.splice(0, 50);
  console.log('total tsums', 'using tsums', this.gameTsums.length);
  if (this.debug) {
    printMaxScores(this.gameTsums);
  }
  // Remove same Tsums
  removeSameTsumImages(this.gameTsums, 0.87);
  console.log('after remove same tsums', this.gameTsums.length);
  if (this.debug) {
    printMaxScores(this.gameTsums);
  }
  
  loadTsumRotationImages(this.gameTsums, this.debug);
  this.isLoadRotateTsum = true;
  // recheck first 5(4) tsums with rotation
  for (var i = 0; i < this.gameTsums.length && i < 8; i++) {
    for (var j = 0; j < this.gameTsums[i].rotations.length; j++) {
      var tsumImage = this.gameTsums[i].rotations[j];
      var xyScore = findImage(boardImg, tsumImage);
      if (xyScore.score > this.gameTsums[i].score) {
        this.gameTsums[i].score = xyScore.score;
      }
    }
  }
  this.gameTsums.sort(function(a, b){
    return a.score > b.score ? -1 : 1;
  });
...
Tsum.prototype.releaseRotationTsum = function() {
...
Tsum.prototype.run = function() {
  ...
  this.tap(Button.gamePause);
  log('辨識盤面Tsum...');
  for (var i = 0; i < Config.tsumCount && i < this.gameTsums.length; i++) {
    for (var j = 0; j < Config.rotations.length; j++) {
      var rotatedImage = this.gameTsums[i].rotations[j];
      var scoreLimit = (this.gameTsums[i].score - 0.5) * 0.7;
