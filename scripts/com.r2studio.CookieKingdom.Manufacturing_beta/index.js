config = {
  ...
  jobFailedCount: 0,
  lastCollectCandyTime: Date.now(),
  lastCollectMail: Date.now(),
  lastCollectFountain: Date.now(),
  lastCollectTrain: Date.now(),
  lastCollectDailyReward: Date.now(),
  run: true,
  isXR: true,
...
function start(inputConfig) {
  ...
  if (!checkIsPage(pageInProduction) && config.autoCollectDailyReward) {
  ...
  if (!checkIsPage(pageInProduction) && config.isCollectCandy) {
  ...
  for (var i = 1; i < 100000000; i++) {
    ...
    console.log('>> ', config.autoCollectMailIntervalInMins, (Date.now() - config.lastCollectMail) / 60000)
    if (
      config.autoCollectMailIntervalInMins != -1 &&
    ...
    console.log('>>> ', config.autoCollectDailyReward, config.lastCollectDailyReward, (Date.now() - config.lastCollectDailyReward) / 60000)
    if (config.autoCollectDailyReward && (Date.now() - config.lastCollectDailyReward) / 60000 > 86400) {
    ...
    if (
      config.autoCollectTrainIntervalInMins != -1 &&
    ...
    console.log('>>>> ', config.autoCollectFountainIntervalInMins, config.lastCollectFountain, (Date.now() - config.lastCollectFountain) / 60000)
    if (
      config.autoCollectFountainIntervalInMins != -1 &&
