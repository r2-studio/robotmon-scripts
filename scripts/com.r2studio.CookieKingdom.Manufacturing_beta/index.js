config = {
    ...
    lastCollectDailyReward: new Date(),
    run: true,
    isXR: true,
  ...
  function start(inputConfig) {
    ...
    if (config.autoCollectDailyReward) {
    ...
    for (var i = 1; i < 100000000; i++) {
      ...
      if (
        config.autoCollectMailInterval != -1 &&
        (Date.now() - config.lastCollectMail) / 60000 > config.autoCollectMailInterval
      ) {
      ...
      if (
        ...
        (Date.now() - config.lastCollectTrain) / 60000 > config.autoCollectMailInterval
      ) {
